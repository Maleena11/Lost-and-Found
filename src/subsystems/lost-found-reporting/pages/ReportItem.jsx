...
export default function ReportItem() {
  ...
  const handleNext = () => {
    ...
    if (currentStep === 1) {
      if (validateStep1()) {
        ...
        setMessage({ text: "Please fill in all required fields before continuing.", type: "error" });
      }
    } else if (currentStep === 2) {
      if (departmentError) {
        setMessage({ text: "Please fix the department selection before continuing.", type: "error" });
        return;
      }
      if (validateStep2()) {
        setCurrentStep(3);
  ...
  const steps = [
    ...
    { label: "Academic & Contact", icon: "fa-university" },
            ...
            <div className="hidden lg:flex flex-col items-center gap-4 flex-shrink-0">
              <div className="relative">
                ...
                <div className="absolute -bottom-2 -left-2 w-7 h-7 bg-amber-400 rounded-full flex items-center justify-center shadow-md border-2 border-amber-300">
                  <i className="fas fa-flag text-white text-[10px]"></i>
                </div>
              </div>
      ...
      <main className="flex-1 w-full flex justify-center items-start px-4 py-8">
        <div className="w-full max-w-3xl">
                ...
                return (
                  <div key={stepNum} className="flex items-center flex-1">
                    <div className="flex flex-col items-center">
                      ...
                      <span className={`text-xs mt-1.5 font-medium whitespace-nowrap ${isActive ? 'text-blue-600' : isCompleted ? 'text-green-600' : 'text-gray-400'}`}>
                        {step.label}
                      </span>
                    </div>
                    {idx < steps.length - 1 && (
                      <div className={`flex-1 h-0.5 mx-3 mb-4 transition-all duration-300 ${currentStep > stepNum ? 'bg-green-400' : 'bg-gray-200'}`} />
                    )}
                  </div>
                );
              })}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            {/* ── STEP 1: Report Type + Item Details ── */}
            {currentStep === 1 && (
              <>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <label className={`flex flex-col items-center justify-center p-3 rounded-2xl border-2 cursor-pointer transition-all duration-200 group ${
                      ...
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center mb-2 transition-all ${formData.itemType === 'lost' ? 'bg-red-100' : 'bg-gray-100 group-hover:bg-red-50'}`}>
                        <i className={`fas fa-search-minus text-base ${formData.itemType === 'lost' ? 'text-red-500' : 'text-gray-300 group-hover:text-red-300'}`}></i>
                      </div>
                      <span className={`font-bold text-xs ${formData.itemType === 'lost' ? 'text-red-700' : 'text-gray-500'}`}>I Lost an Item</span>
                      {formData.itemType === 'lost' && (
                    ...
                    <label className={`flex flex-col items-center justify-center p-3 rounded-2xl border-2 cursor-pointer transition-all duration-200 group ${
                      ...
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center mb-2 transition-all ${formData.itemType === 'found' ? 'bg-green-100' : 'bg-gray-100 group-hover:bg-green-50'}`}>
                        <i className={`fas fa-hand-holding text-base ${formData.itemType === 'found' ? 'text-green-500' : 'text-gray-300 group-hover:text-green-300'}`}></i>
                      </div>
                      <span className={`font-bold text-xs ${formData.itemType === 'found' ? 'text-green-700' : 'text-gray-500'}`}>I Found an Item</span>
                      {formData.itemType === 'found' && (
                ...
                <div className="flex justify-end">
                  <button
                    type="button"
                    onClick={handleNext}
                    className="px-8 py-3.5 rounded-xl font-bold text-white bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 transition-all duration-200 text-sm flex items-center gap-2 shadow-lg shadow-blue-200"
                  >
                    Next
            ...
            {/* ── STEP 2: Academic Info + Contact Info ── */}
            {currentStep === 2 && (
              <>
                ...
                <div className="flex gap-4">
                  <button
                    type="button"
                    onClick={handleBack}
                    className="flex-1 py-3.5 rounded-xl font-semibold text-gray-700 bg-white border border-gray-200 hover:bg-gray-50 transition-all duration-200 text-sm flex items-center justify-center gap-2 shadow-sm"
                  >
                  ...
                  <button
                    type="button"
                    onClick={handleNext}
                    className="flex-2 px-8 py-3.5 rounded-xl font-bold text-white bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 transition-all duration-200 text-sm flex items-center justify-center gap-2 shadow-lg shadow-blue-200"
                  >
                    Next
                    <i className="fas fa-arrow-right"></i>
                  </button>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                  ...
                  <div className="flex gap-4">
                    ...
                    <button
                      ...
                      className={`flex-2 px-8 py-4 rounded-xl font-bold text-white shadow-lg transition-all duration-200 flex items-center justify-center gap-2.5 text-sm ${
                        formData.itemType === 'lost'
